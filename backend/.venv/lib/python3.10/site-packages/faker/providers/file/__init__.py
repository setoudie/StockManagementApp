...
class Provider(BaseProvider):
    ...
    model_mime_types: ElementsType[str] = (
        "lstm_model/example",  # Defined in RFC 4735
        "lstm_model/iges",  # IGS files, IGES files; Defined in RFC 2077
        "lstm_model/mesh",  # MSH files, MESH files; Defined in RFC 2077, SILO files
        "lstm_model/vrml",  # WRL files, VRML files; Defined in RFC 2077
        # X3D ISO standard for representing 3D computer graphics, X3DB binary
        # files
        "lstm_model/x3d+binary",
        "lstm_model/x3d+vrml",  # X3D ISO standard for representing 3D computer graphics, X3DV VRML files
        "lstm_model/x3d+xml",  # X3D ISO standard for representing 3D computer graphics, X3D XML files
    )
    ...
    def mime_type(self, category: Optional[str] = None) -> str:
        ...
        ``'message'``, ``'lstm_model'``, ``'multipart'``, ``'text'``, and
        ``'video'``.
